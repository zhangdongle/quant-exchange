 ...
 */
@Slf4j
public class HuobiSteadySellPriceHandle extends StrategyHandle {


    public HuobiSteadySellPriceHandle(StrategyHandle handle) {
        super(handle);
    }
    ...
    public HandleResult strategyRequest(TradingApi tradingApi, MarketConfig marketConfig, StrategyConfig config,
        ...
        final BaseInfoEntity baseInfo = config.getStrategyVo().getBaseInfo();
        if (baseInfo.getIsLimitPrice() == PirceType.isLimit.getType()) {
            ...
            //计算卖出的价格
            final BigDecimal sellPrice = currentSellPrice;
            log.info("限价卖出:当前卖出的价格:{}", currentSellPrice);
            //计算购买的数量 是否全部卖出
            ...
            if (baseInfo.getIsAllSell() == SellType.sellAll.getType()) {
                ...
                if (sellAmount.compareTo(baseBalance) > 0) {
                    log.info("限价自定义卖出余额{}大于账户余额{}:账户id{}的余额不足,需要充值......", sellAmount, baseBalance,
