...
public class HuoBIndicatoryStrategyImpl extends AbstractStrategy implements TradingStrategy, StrategyDelegate {
    ...
    public void execute() throws StrategyException {
        ...
        while (true) {
            try {
                ...
                Bar newBar = new BaseBar(Duration.ofMinutes(1), nowTime, timeSeries.function());
                //                Bar newBar = new BaseBar(nowTime, line.getOpen(), line.getHigh(), line.getLow(), line.getClose(),
                //                        line.getVol(), timeSeries.function());
                ...
                if (strategy.shouldExit(endIndex, tradingRecord) && orderState.getType() == OrderType.BUY) {
                    //查看是否达到卖的信号
                    boolean exit = tradingRecord.exit(endIndex, newBar.getClosePrice(), DecimalNum.valueOf(1));
                    //                    boolean exit = tradingRecord.exit(endIndex, newBar.getClosePrice(), DecimalNum.valueOf(1));
                    if (exit) {
                        try {
                            createSellOrder();
                ...
                //查看是否到达买的信号
                    ...
                    boolean enter = tradingRecord.enter(endIndex, newBar.getClosePrice(), DecimalNum.valueOf(1));
                    if (enter) {
                        try {
                            createBuyOrder();
                        } catch (Exception e) {
        ...
        if (this.orderState.getType() != null) {
            ...
            if (this.orderState.getType() == OrderType.BUY) {
                ...
                if (this.orderState.getPrice() == null) {
                    ...
                    tradingRecord.enter(1999, DecimalNum.valueOf(
                            this.orderState.getPrice().setScale(pricePrecision, RoundingMode.DOWN).doubleValue()),
            ...
            if (first.isPresent()) {
                ...
                if (cancel) {
                    redisMqService.sendMsg(
                            "查询到未成功的订单,orderId【" + first.get().getId() + "】, 取消【" + this.orderState.getType()
                                    .getStringValue() + "】订单成功!!!");
                    if (this.orderState.getType() == OrderType.BUY) {
    ...
    public void CalculateProfit() {
        try {
            ...
            if (this.orderState.getType() == OrderType.SELL) {
                ...
                log.info("盈亏率:{}", divide);
                profitMessage(buyOrderId, sellOrderId, buyAmount, sellAmount, buyPrice, sellPrice, diff, divide,
                        ordersBuyDetail, ordersSellDetail);
    ...
    private void profitMessage(long buyOrderId, long sellOrderId, BigDecimal buyAmount, BigDecimal sellAmount,
            BigDecimal buyPrice, BigDecimal sellPrice, BigDecimal diff, BigDecimal divide, OrdersDetail ordersBuyDetail,
