...
public abstract class AbstractStrategy {
    ...
    public AbstractStrategy(RedisUtil redisUtil, Integer robotId) {
     ...
     * @param HBOrderType
     */
    protected void order(BigDecimal amount, BigDecimal price, HBOrderType HBOrderType, TradingApi tradingApi,
            OrderType type) {
        //下订单
        try {
            redisMqService.sendMsg("叮叮叮>>>开始下单,下单信息 价格:" + price + "数量:" + amount + "订单类型:" + HBOrderType.getTyoe());
            this.orderState.setId(tradingApi
                    .createOrder(this.marketConfig.markName(), this.accountConfig.accountId(), HBOrderType, amount,
                            price));
    ...
    protected boolean getQuotaPriceAndPrecision() {
        String quota_price_amount = String
                .valueOf(redisUtil.get(RobotRedisKeyConfig.getSymbol() + marketConfig.markName()));
        ...
        redisMqService.sendMsg(
                "当前机器人的基础币种{" + baseCurrency + "} 配额币种{" + quotaCurrency + "} 价格的精度{" + pricePrecision + "} 数量的精度{"
                        + amountPrecision + "}");
        return true;
    }
    ...
    public static class OrderState {
        ...
        public String toString() {
            return MoreObjects.toStringHelper(this).add("id", id).add("type", type).add("price", price)
    ...
    public class Profit {
        ...
        public Profit(OrdersDetail ordersBuyDetail, OrdersDetail ordersSellDetail, OrderState orderState) {
        ...
        public Profit invoke() {
            if (orderState.getHBOrderType() == HBOrderType.SELL_MARKET) {
                ...
                buyPrice = new BigDecimal(ordersBuyDetail.getFieldCashAmount())
                        .divide(new BigDecimal(ordersBuyDetail.getFieldAmount()), pricePrecision, RoundingMode.DOWN);
                sellPrice = new BigDecimal(ordersSellDetail.getFieldCashAmount())
                        .divide(new BigDecimal(ordersSellDetail.getFieldAmount()), pricePrecision, RoundingMode.DOWN);
            } else {
