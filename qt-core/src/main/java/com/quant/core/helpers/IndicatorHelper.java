    ...
    public static BarSeries buildSeries(List<Kline> lines) {
        ...
        for (Kline kline : lines) {
            ZonedDateTime time = ZonedDateTime
                    .ofInstant(Instant.ofEpochMilli(kline.getId() * 1000), ZoneId.systemDefault());
            // build a bar
            Bar bar = new BaseBar(Duration.ofMinutes(1), time, series.function());
//            Bar bar = new BaseBar(time, kline.getOpen(), kline.getHigh(), kline.getLow(), kline.getClose(),
//                    kline.getVol(), series.function());
            series.addBar(bar);
        }
        return series;
     ...
     * @return
     */
    public static Rule simpleBuilder(IndicatorBean bean, IndicatorFactory factory, BarSeries series, Rule rule,
            ThreadLocal<Boolean> onlyOneRule) {
        //指标名称
     ...
     * @return
     */
    public static Rule builderRule(boolean price, boolean amount, IndicatorFactory factory, String compare,
            String condition, String value, ThreadLocal<Boolean> ruleOnlyOne, IndicatorBean bean, BarSeries timeSeries,
            Rule entry) {
        if (price) {
            //是价格指标
            ...
            if (params2 == null || params2.equals("") || name.equals("number")) {
                ...
                if (compare.equals(indicator_cross_down)) {
                    //大于等于
                    CrossedDownIndicatorRule crossedDownIndicatorRule = new CrossedDownIndicatorRule(indicatorFirst,
                            indicatorSecond);
                    entry = andOr(entry, crossedDownIndicatorRule, ruleOnlyOne.get(), condition);
                }
                if (compare.equals(indicator_cross_up)) {
                    //小于等于
                    CrossedUpIndicatorRule crossedUpIndicatorRule = new CrossedUpIndicatorRule(indicatorFirst,
                            indicatorSecond);
    ...
    private static Rule AddOrAnd(Indicator volumeIndicator, Rule entry, String compare,
            ThreadLocal<Boolean> ruleOnlyOne, String condition, String value) {
